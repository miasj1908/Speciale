...
OUTPUT_FILE = r"C:\Users\miasj\PythonProjects\equity_reports_update_dagsdiff2.xlsx"
...
# Foretrukken folder path: Rådata - virksomheder, ellers første tilgængelige
def pick_folder_path(group):
    rådata = group[group["Folder Path"].str.contains("Rådata - virksomheder", na=False)]
    return rådata["Folder Path"].iloc[0] if not rådata.empty else group["Folder Path"].iloc[0]

cf_best_path = df_cf.groupby(["Company", "Date", "Cash Flow Model"], group_keys=False).apply(
    pick_folder_path
).reset_index(name="Folder Path")

# Unikke (Company, Date) med CF-modelnavn og folder path
cf_unique = cf_best_path.drop_duplicates(subset=["Company", "Date"]).copy()
...
for _, rep in df_reports.iterrows():
    ...
    if candidates.empty:
        ...
        cf_model_names.append(None)
        cf_folder_paths.append(None)
    else:
        diffs  = (candidates["Date"] - rep["report_date"]).abs()
        best   = candidates.loc[diffs.idxmin()]
        nearest_cf_dates.append(best["Date"])
        days_diffs.append((best["Date"] - rep["report_date"]).days)
        cf_model_names.append(best["Cash Flow Model"])
        cf_folder_paths.append(best["Folder Path"])

df_reports["nearest_cf_date"] = nearest_cf_dates
df_reports["days_diff"]        = days_diffs
df_reports["cf_model"]         = cf_model_names
df_reports["cf_folder_path"]   = cf_folder_paths
...
new_cols = ["nearest_cf_date", "days_diff", "cf_model", "cf_folder_path"]
new_vals = [nearest_cf_dates, days_diffs, cf_model_names, cf_folder_paths]

for offset, (col_name, col_vals) in enumerate(zip(new_cols, new_vals)):
    col_idx = max_col + offset
    cell = ws.cell(row=1, column=col_idx, value=col_name)
    cell.fill = header_fill
    cell.font = header_font
    for i, val in enumerate(col_vals, start=2):
        if col_name == "days_diff":
            ws.cell(row=i, column=col_idx, value=int(val) if pd.notna(val) else None)
        elif col_name == "nearest_cf_date":
            ws.cell(row=i, column=col_idx, value=val if pd.notna(val) else None)
        else:
