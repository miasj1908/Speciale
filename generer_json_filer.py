...
To JSON filer per forecast:
  - _input.json  : kun historiske tal (sendes til LLM)
  - _facit.json  : analytikerprognoser + realiserede tal (bruges til evaluering)
...
# Mapper hvor JSON filer gemmes
OUTPUT_MAPPE        = r"C:\Users\miasj\PythonProjects\json_filer_CF"
OUTPUT_MAPPE_INPUT  = os.path.join(OUTPUT_MAPPE, "input")
OUTPUT_MAPPE_FACIT  = os.path.join(OUTPUT_MAPPE, "facit")
...
def hent_regnskabsdata(cur, forecast_id, datakilde, min_år=None, max_år=None):
    """Henter alle regnskabstal for én forecast og datakilde."""
    if min_år is not None and max_år is not None:
        cur.execute("""
            SELECT sektion, line_item, årstal, værdi
            FROM regnskabsdata
            WHERE forecast_id = %s AND datakilde = %s AND årstal >= %s AND årstal <= %s
            ORDER BY sektion, line_item, årstal
        """, (forecast_id, datakilde, min_år, max_år))
    else:
        cur.execute("""
            SELECT sektion, line_item, årstal, værdi
            FROM regnskabsdata
...
def generer_json_filer():
    ...
    # Opret output mapper
    os.makedirs(OUTPUT_MAPPE_INPUT, exist_ok=True)
    os.makedirs(OUTPUT_MAPPE_FACIT, exist_ok=True)
    ...
    for idx, forecast in enumerate(forecasts):
        ...
        try:
            ...
            historisk  = hent_regnskabsdata(cur, forecast_id, 'historisk', min_år=forecast_dato.year - 6, max_år=forecast_dato.year - 1)
            analytiker = hent_regnskabsdata(cur, forecast_id, 'analytiker', min_år=forecast_dato.year, max_år=forecast_dato.year + 2)
            ...
            # Input fil — kun historiske tal (sendes til LLM)
            input_data = {**meta, "historisk": historisk}
            input_fil = os.path.join(OUTPUT_MAPPE_INPUT, f"{ticker}_{str(forecast_dato).replace('-', '')}_input.json")
            with open(input_fil, 'w', encoding='utf-8') as f:
                json.dump(input_data, f, ensure_ascii=False, indent=2,
                         default=json_serializer)

            # Facit fil — analytiker + realiseret (bruges til evaluering)
            facit_data = {**meta, "analytiker": analytiker, "realiseret": realiseret}
            facit_fil = os.path.join(OUTPUT_MAPPE_FACIT, f"{ticker}_{str(forecast_dato).replace('-', '')}_facit.json")
    ...
    print("FAERDIG")
    print("="*50)
    print(f"Forecasts behandlet:  {succes} ({succes * 2} JSON filer)")
    print(f"Fejl:                 {fejl}")
    print(f"Input filer gemt i:   {OUTPUT_MAPPE_INPUT}")
    print(f"Facit filer gemt i:   {OUTPUT_MAPPE_FACIT}")

    # Vis eksempel på første fil
    if succes > 0:
        print("\n=== EKSEMPEL PAA FOERSTE JSON FIL ===")
        første_fil = os.listdir(OUTPUT_MAPPE_INPUT)[0]
        with open(os.path.join(OUTPUT_MAPPE_INPUT, første_fil), 'r') as f:
            data = json.load(f)
